...
def fetch_quote(name: str, key: str | None = None) -> dict | None:
    """
    Live daily quote for one company: last traded price (₹) and market cap
    (₹ crore), pulled from IndianAPI (the same NSE feed Screener shows).

    Returns None on any failure (no key, network error, rate limit, company not
    found) so the caller can silently fall back to the workbook's own figures.
    The value is a point-in-time daily quote — cache it for a day upstream.
    """
    if not name:
        return None
    try:
        ...
        return None                      # no INDIANAPI_KEY configured
    try:
        raw = fetcher.fetch(name)
    except IndianAPIError:
        return None                      # auth / rate limit — fall back quietly
    if not isinstance(raw, dict):
        return None

    reuse = raw.get("stockDetailsReusableData") or {}
    cp = raw.get("currentPrice") or {}
    # NSE is the primary board; fall back to BSE, then the reusable close.
    price = _f(cp.get("NSE")) or _f(cp.get("BSE")) or _f(reuse.get("price")) or _f(reuse.get("close"))
    mcap = _f(reuse.get("marketCap"))
    if price is None and mcap is None:
        return None

    quote: dict[str, Any] = {"source": "IndianAPI (NSE live)"}
    if price is not None:
        quote["current_price"] = price
    if mcap is not None:
        quote["market_cap"] = mcap
    as_of = reuse.get("date") or ""
    if as_of:
        quote["as_of"] = str(as_of)
    return quote
