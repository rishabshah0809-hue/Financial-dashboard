...
def _model_interpretation_block(model, sector_key: str, config: LLMConfig,
    ...
    for skey, heading in INTERP_SECTIONS:
        ...
        # The corner deco is a direct child of the card (not of the text body), so
        # it always sits flush in the card's bottom-right corner regardless of how
        # much text the body holds — never cut off or stuck to the text.
        cards.append(
            f'<div class="icard{wide}" style="--hue:{hue};--tint:{tint}">{icon}'
            f'<div class="ih">{heading}</div>'
            f'<div class="ibody">{body}</div>{_interp_deco(hue)}</div>'
        )
